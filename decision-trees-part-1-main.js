...
var FILL_FN = function(d, o) {
    ...
    if (
        (typeof d == "object" && d["target"] > 0.5) ||
        d === "isTarget" ||
        (typeof d == "boolean" && d)
    ) {
        // return "rgba(0,122,61," + o + ")"; // SF
        // return "rgba(245,100,30," + o + ")"; // Giants Orange
        // return "rgba(245,170,60," + o + ")"; // Warriors Gold
        // return "rgba(180,90,20," + o + ")"; // Warriors Gold, slightly darkened
        // return "rgba(200,100,20," + o + ")"; // Another Orange
        // return "rgba(200,100,20," + o + ")"; // Another Orange
        return "rgba(0,122,61," + o + ")"; // Another Green
    } else {
        // return "rgba(245,160,80," + o + ")"; // NYC
        // return "rgba(200,100,20," + o + ")"; // Yankee Blue
        // return "rgba(245,130,32," + o + ")"; // Knicks Blue
        return "rgba(245,130,32," + o + ")"; // Another Blue
    }
};
    ...
    for (i = 0; i < dimensionsArray.length; i++) {
        for (j = i + 1; j < dimensionsArray.length; j++) {
            ...
            */
            ...
            var animationOrdinality =
                d * 2 + diagonals[d] + i - j + dimensionsArray.length;

            var totalPlots =
                (dimensionsArray.length * (dimensionsArray.length - 1)) / 2;
...
var ComputePrecisePlotAxis = function(precisePlots, dimensionsArray) {
    ...
    precisePlots.forEach(function(plot) {
        if (
            verticalAxis.length < plot.abstractY + 1 &&
            plot.abstractX == dimensionsArray.length - 2
        ) {
            verticalAxis.push({
...
var ComputePreciseScatterPlotPositions = function(
    totalLength,
    scrollExtent,
    dataDimensions,
    AbstractScatterPlotsAarray,
    dataPoints
) {
    // Geometry Setup
    var margin = 30;
    var padding = 15;
    var sideLength = Math.floor(
        (totalLength - (dataDimensions.length - 1) * padding - margin) /
    ...
    _.each(AbstractScatterPlotsAarray, function(plot, i) {
        ...
        var scrollStart =
            scrollExtent[0] * (1 - plot.abstractStart) +
            scrollExtent[1] * plot.abstractStart;
        var scrollEnd =
            scrollExtent[0] * (1 - plot.abstractEnd) +
            scrollExtent[1] * plot.abstractEnd;
...
var ComputeTestTree = function(tree, test_set) {
    ...
    var partitionFork = function(tree, data, depth) {
        ...
        var gini =
            1 -
            (isTargetLength * isTargetLength + isNotTargetLength * isNotTargetLength);
...
var ScrollHinterView = Backbone.View.extend({
    initialize: function(args) {
        ...
        this.opacityFn = d3.scale
            .linear()
            .domain([0, 200])
            .range([1, 0])
            .clamp(true);

        this.topMarginFn = d3.scale
            .linear()
            .domain([0, 200])
            .range([0, 200])
            .clamp(true);
    ...
    render: function() {
        this.el.attr(
            "style",
            "top: " + this.top + "px; opacity: " + this.opacity + ";"
        );
...
var HousingAxisHistogram = Backbone.View.extend({
    ...
    handleResize: function(args) {
        ...
        if (this.split) {
            ...
            this.split_location =
                (this.valueEdge / (extent[1] - extent[0])) * (this.split - extent[0]);
        }
...
var StaticScatterPlotView = Backbone.View.extend({
    initialize: function(args) {
        ...
        this.plots = ComputePreciseScatterPlotPositions(
            600,
            this.scrollExtent,
...
var HousingScatterPlotMatrixCanvas = Backbone.View.extend({
    initialize: function(args) {
        ...
        // Data Prep
        plots = ComputeAbstractScatterPlotPositions(this.dimensionsArray);
        this.data.plots = ComputePreciseScatterPlotPositions(
            600,
            this.scrollExtent,
            this.dimensionsArray,
            plots,
        ...
        this.data.axis = ComputePrecisePlotAxis(
            this.data.plots,
            this.dimensionsArray
        );
    ...
    handleResize: function(args) {
        ...
        */
        ...
        var animationFinish =
            $morevariables.offset().top +
            $morevariables.height() / 2 -
            windowHeight * 0.75;
        ...
        var plots = ComputePreciseScatterPlotPositions(
            this.canvasLength,
            this.scrollExtent,
            this.dimensionsArray,
            this.data.plots,
...
var StaticPlotRetreat = Backbone.View.extend({
    ...
    render: function() {
        this.el.attr(
            "style",
            "transform: translate(0, " + this.staticPlotY + "px)"
        );
    },
});
...
var DimensionPlotCanvas = Backbone.View.extend({
    ...
    clean: function() {
        ...
        this.context.fillStyle =
            "rgb(" + offWhite + ", " + offWhite + ", " + offWhite + ")";
        this.context.rect(
            this.minX - 3,
            this.minY - 3,
            this.canvasWidth + 6,
...
var ScatterPlotPointCanvas = Backbone.View.extend({
    initialize: function(args) {
        ...
        this.interpolator = (function(v) {
            ...
            var interpolateO = d3.interpolateNumber(
                v.data.start.opacity,
                v.data.end.opacity
            );
...
var ScatterPlotPointBars = Backbone.View.extend({
    ...
    handleResize: function(args) {
        ...
        */
        ...
        this.scrollExtents = {
            ...
            rise: [
                riseStarts + riseRange * this.orderNormalized,
                riseStarts + duration + riseRange * this.orderNormalized,
            ],
            shorten: [
                shortenStarts + shortenRange * yNormalized,
                shortenStarts + duration * 12 + shortenRange * yNormalized,
            ],
            scatter: [
                scatterStarts + scatterRange * yNormalized,
                scatterStarts + duration * 12 + scatterRange * yNormalized,
            ],
            shrink: [
                shrinkStarts + staggerRange * (1 - this.orderNormalized),
                shrinkStarts +
                duration * 12 +
                staggerRange * (1 - this.orderNormalized),
            ],
        };
    },
    handleScroll: function(scroll) {
        ...
        if (scroll < this.scrollExtents.shrink[0]) {
            // y position
            if (
                scroll > this.scrollExtents.rise[0] &&
                scroll <= this.scrollExtents.rise[1]
            ) {
                var progress =
                    (scroll - this.scrollExtents.rise[0]) /
                    (this.scrollExtents.rise[1] - this.scrollExtents.rise[0]);
            ...
            // width
            if (
                scroll > this.scrollExtents.shorten[0] &&
                scroll <= this.scrollExtents.shorten[1]
            ) {
                var progress_shorten =
                    (scroll - this.scrollExtents.shorten[0]) /
                    (this.scrollExtents.shorten[1] - this.scrollExtents.shorten[0]);
            ...
            // x position
            if (
                scroll > this.scrollExtents.scatter[0] &&
                scroll <= this.scrollExtents.scatter[1]
            ) {
                var progress_scatter =
                    (scroll - this.scrollExtents.scatter[0]) /
                    (this.scrollExtents.scatter[1] - this.scrollExtents.scatter[0]);
            ...
            // Phase 2
            if (
                scroll > this.scrollExtents.shrink[0] &&
                scroll <= this.scrollExtents.shrink[1]
            ) {
                var progress_shrink =
                    (scroll - this.scrollExtents.shrink[0]) /
                    (this.scrollExtents.shrink[1] - this.scrollExtents.shrink[0]);
    ...
    render: function() {
        var transformString =
            "translate(" + this.x + ", " + this.y + ") rotate(" + this.rotate + ")";
...
var VariableIntroductionView = Backbone.View.extend({
    initialize: function(args) {
        ...
        var xAxisKey = "soil_depth";
        var yAxisKey = "stump density";

        this.xDomain = d3.extent(
            this.plot.points,
            function(d) {
        ...
        this.elevationMarkersData = [{
            ...
            },
            {
                value: isTargetY_Max,
                normalized:
                    (isTargetY_Max - overallY_extent[0]) /
                    (overallY_extent[1] - overallY_extent[0]),
                x: -200,
                y: -200,
            },
            {
                ...
                normalized:
                    (isNotTargetY_Max - overallY_extent[0]) /
                    (overallY_extent[1] - overallY_extent[0]),
                x: -200,
                y: -200,
            },
        ...
        this.elevationMarkers = this.overlayGroup
            .selectAll(".elevationMarkerGroup")
        ...
        this.psqfMarkersData = [{
            ...
            },
            {
                value: isTargetX_Max,
                normalized:
                    (isTargetX_Max - overallX_extent[0]) /
                    (overallX_extent[1] - overallX_extent[0]),
                x: -200,
                y: -200,
            },
            {
                ...
                normalized:
                    (isNotTargetX_Max - overallX_extent[0]) /
                    (overallX_extent[1] - overallX_extent[0]),
                x: -200,
                y: -200,
            },
        ...
        this.psqfMarkers = this.overlayGroup
            .selectAll(".psqfMarkerGroup")
    ...
    handleResize: function(args) {
        ...
        var riseFinish =
            $firsttwo.offset().top + $firsttwo.height() / 2 - windowHeight * 0.55;

        var scatterStart = $firsttwo.offset().top + 10;
        var scatterFinish =
            $addnuance.offset().top + $addnuance.height() / 2 - windowHeight * 0.55;

        var boundaryStart = $addnuance.offset().top + 10;
        var boundaryFinish =
            $setboundaries.offset().top +
            $setboundaries.height() / 2 -
            windowHeight * 0.55;
        ...
        this.overlayGroup.attr(
            "transform",
            "translate(" + (padding + 3) + ", " + (padding + 3) + ")"
        );
        ...
        this.opacityFn.domain([
            this.shrinkStart,
            this.shrinkStart + windowHeight / 4,
        ]);
    },
    handleScroll: function(scroll) {
        var v = this;
        ...
        this.elevationMarkersData.forEach(function(d) {
            ...
            var y =
                this.sideLength - scrollProgress * this.sideLength * d.normalized + 4;
...
var SplitQualityView = Backbone.View.extend({
    initialize: function(args) {
        ...
        // Geometery Prep
        this.interpolator = d3.interpolateNumber(
            this.stats.min_val,
            this.stats.max_val
        );
    ...
    setSplitPoint: function(splitValue) {
        if (!this.splitData || splitValue != this.splitData.splitValue) {
            ...
            if (this.accuracyIndicator) {
                var correct =
                    this.splitData.gt.isTarget.length +
                    this.splitData.lte.isNotTarget.length;
                var total =
                    this.splitData.gt.isTarget.length +
                    this.splitData.gt.isNotTarget.length +
                    this.splitData.lte.isTarget.length +
                    this.splitData.lte.isNotTarget.length;
    ...
    handleResize: function(args) {
        ...
        this.opacityThresholds = d3.scale
            .linear()
            .domain(domain)
            .range([1, 0, 0])
            .clamp(true);

        this.yThresholds = d3.scale
            .linear()
            .domain(domain)
            .range([windowHeight * 0.02, windowHeight * -0.4, windowHeight * -0.4])
            .clamp(true);

        var indicatorXfn = d3.scale
            .linear()
            .domain([this.stats.min_val, this.stats.max_val])
            .range([this.padding, this.canvasLength - this.padding]);

        this.splitIndicator.handleResize(
            this.baseline + this.padding,
            indicatorXfn
        );
    ...
    render: function() {
        ...
        this.gtLayer.attr(
            "transform",
            "translate(" +
            this.canvasLength * 0.8 +
            ", " +
            this.pieVerticalOffset +
            ")"
        );
...
var ElevationToHistogramView = Backbone.View.extend({
    initialize: function(args) {
        ...
        this.rotationInterpolator = d3.interpolateNumber(
            this.rotateRange[0],
            this.rotateRange[1]
        );
    ...
    handleResize: function(args) {
        ...
        this.elevationScale = d3.scale
            .linear()
            .domain(this.domain)
            .range(this.range);

        this.heightScale = d3.scale
            .linear()
    ...
    handleScroll: function(scroll) {
        ...
        // Rotation
        if (
            scroll > this.scrollSections.rotate[0] &&
            scroll <= this.scrollSections.rotate[1]
        ) {
            this.rotate.target = this.rotationInterpolator(
                (scroll - this.scrollSections.rotate[0]) /
                (this.scrollSections.rotate[1] - this.scrollSections.rotate[0])
            );
        } else {
...
var ElevationRainDrop = Backbone.View.extend({
    ...
    handleResize: function(args) {
        ...
        var shrinkDuration =
            (this.scrollSections.shrink[1] - this.scrollSections.shrink[0]) / 2;
        var shrinkDelay = shrinkDuration;
        var shrinkFactor =
            1 -
            (this.data[this.key] - this.domain[0]) /
            (this.domain[1] - this.domain[0]);

        var fallDuration =
            (this.scrollSections.fall[1] - this.scrollSections.fall[0]) / 6;
        var fallDelay =
            (this.scrollSections.fall[1] -
                this.scrollSections.fall[0] -
                fallDuration) /
            350;
        ...
        if (this.data.target < 0.5) {
            ...
            this.yFn = d3.scale
                ...
                .range([this.padding, this.padding, this.padding, bottom])
                .clamp(true);
        }
        ...
        this.heightFn = d3.scale
            ...
            .range([this.fullHeight, this.minHeight])
            .clamp(true);
...
var SplitHistogramComparisonView = Backbone.View.extend({
    initialize: function(args) {
        ...
        var rightSideAttribute =
            DIMENSION_LONGFORM[tree_stats[gtNodeID].attribute][LANG];
        var rightSideValue =
            DIMENSION_UNITS[tree_stats[gtNodeID].attribute].prefix +
            Math.round(parseFloat(tree_stats[gtNodeID].split_point)) +
            " " +
            DIMENSION_UNITS[tree_stats[gtNodeID].attribute].suffix;
        ...
        var leftSideAttribute =
            DIMENSION_LONGFORM[tree_stats[ltNodeID].attribute][LANG];
        var leftSideValue =
            DIMENSION_UNITS[tree_stats[ltNodeID].attribute].prefix +
            Math.round(parseFloat(tree_stats[ltNodeID].split_point)) +
            " " +
            DIMENSION_UNITS[tree_stats[ltNodeID].attribute].suffix;
    ...
    handleResize: function(args) {
        ...
        this.yThresholds = d3.scale
            .linear()
            .domain(domain)
            .range([windowHeight * 0.8, windowHeight * 0.5, windowHeight * 0.5])
            .clamp(true);
...
var GiniPieChartView = Backbone.View.extend({
    ...
    render: function() {
        ...
        this.isNotTargetNumberText.attr(
            "transform",
            "translate(-70," + (this.radius + 40) + ")"
        );

        this.isTargetNumberText.attr(
            "transform",
            "translate(70," + (this.radius + 40) + ")"
        );
...
var SplitIndicator = Backbone.View.extend({
    ...
    render: function() {
        var displayNumber = UNIT_CONVERSION(
            Math.round(this.current.split),
            "m",
            LANG
        );
        this.textNode.text(displayNumber);
...
var DecisionTreeView = Backbone.View.extend({
    initialize: function(args) {
        ...
        this.nodes_layer = this.svg
            .append("g")
            .attr("class", "nodes_layer")
            .attr("transform", "translate(0, " + this.verticalPadding + ")");

        this.nodes = this.nodes_layer
            .selectAll(".node")
...
var TreeNodeView = Backbone.View.extend({
    ...
    handleResize: function(args) {
        ...
        // Histogram Threshold
        this.histogramOpacityThreshold = d3.scale
            .threshold()
            .domain(domain)
        ...
        if (!this.links) {
            ...
            var distanceToGo =
                ((this.maxDepth - this.depth) * (windowHeight * 0.45)) / this.maxDepth;

            this.verticalOffsetThreshold = d3.scale
                .linear()
                .domain([h1, end])
        ...
        this.pieOpacityThreshold = d3.scale
            .threshold()
            .domain(domain)
...
var TreePieView = Backbone.View.extend({
    ...
    computeGeometry: function() {
        ...
        this.area =
            (this.maxArea / this.total_data_count) * parseInt(this.data.total);
...
var ClassifierResultsView = Backbone.View.extend({
    ...
    handleResize: function(args) {
        ...
        this.opacityInterpolator.domain([
            testScrollStart - scrollDuration,
            testScrollStart,
        ]);
        this.handleScroll(this.scroll);
...
var AnimatedClassifierView = Backbone.View.extend({
    ...
    initialize: function(args) {
        ...
        _.each(
            ...
            function(node) {
                ...
                var leftLine = leftArrow
                    .append("path")
                    .attr("fill", "none")
                    .attr("stroke-width", 1);

                var rightLine = rightArrow
                    .append("path")
                    .attr("fill", "none")
                    .attr("stroke-width", 1);
    ...
    updateSamplePath: function(s, si) {
        ...
        _.each(
            ...
            function(p, i) {
                ...
                pathString +=
                    "L " + this.treeScales.x(p.x) + " " + this.treeScales.y(p.y) + " ";
                pathString +=
                    "L " +
                    this.treeScales.x(p.x) +
                    " " +
                    (this.treeScales.y(p.y) + this.labelOffset) +
                    " ";
            },
            this
        );
        ...
        if (s.classification === "isTarget") {
            // Top of Bin
            pathString +=
                "L " + this.treeScales.x(0.75) + " " + this.treeScales.y(1.2) + " ";

            var x =
                this.treeScales.x(1.05) +
                s.pileCoordinates.row * this.ballSpacing * -1 +
                (-this.ballSpacing / 2) * s.pileCoordinates.col; // + 8 * this.ballSpacing;
            ...
            // Top of Bin
            pathString +=
                "L " + this.treeScales.x(0.25) + " " + this.treeScales.y(1.2) + " ";

            var x =
                this.treeScales.x(0.02) +
                s.pileCoordinates.row * this.ballSpacing +
                (this.ballSpacing / 2) * s.pileCoordinates.col;
            var y = s.pileCoordinates.col * this.ballSpacing * -0.86 + baseline;
    ...
    handleResize: function() {
        ...
        var treeExtent = {
            ...
            y: [
                this.bounds.height * 0.05 + this.ballSpacing,
                this.bounds.height * 0.6,
            ],
        };
        ...
        _.each(
            ...
            function(l) {
                ...
                if (!l.node.children) {
                    ...
                    // Lines
                    var midY =
                        this.treeScales.y(l.node.y) + this.labelOffset + this.ballSpacing;

                    var dx0 = x - this.treeScales.x(l.node.children[0].x);
                    var dx1 = x - this.treeScales.x(l.node.children[1].x);
                    var dy =
                        midY - (this.treeScales.y(l.node.children[0].y) + this.ballSpacing);
                    ...
                    var leftD = "";
                    leftD +=
                        "M " + x + " " + (y + this.labelOffset + this.ballSpacing) + " ";
                    leftD += "L " + nx0 + " " + ny0 + " ";
                    leftD +=
                        "L " +
                        nx0 +
                        " " +
                        (this.treeScales.y(l.node.children[0].y) +
                            this.labelOffset -
                            this.ballSpacing) +
                        " ";
        ...
        var trainingInterval = Math.floor(
            (trainingEnd - trainingStart - ballDurationInPixels) /
            this.trainingSamples.length
        );
        var testInterval = Math.floor(
            (testEnd - testStart - ballDurationInPixels) / this.testSamples.length
        );
    ...
    handleScroll: function(scroll) {
        ...
        _.each(
            ...
            function(set) {
                _.each(
                    ["isTarget", "isNotTarget"],
                    function(target) {
                        if (percentages[set][target].total > 0) {
                            percentages[set][target].percent = Math.round(
                                (percentages[set][target].correct /
                                    percentages[set][target].total) *
                                100
                            );
                        } else {
    ...
    renderSamples: function(sample) {
        ...
        if (this.sampleID) {
            if (
                this.sampleID == sample.attributes.index &&
